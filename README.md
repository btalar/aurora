...
As you can see, there is a \_\_tests\_\_/setupVitest.ts to create at the root of the project, it will handle the setup of all the vitest tests.
```typescript
import 'jsdom'
```
Ignore this file from the type generation plugin
```typescript
dts({
  tsconfigPath: fileURLToPath(
    new URL('tsconfig.lib.json', import.meta.url),
  ),
  exclude: ['**/*.stories.ts', '__tests__/*.ts'],
}),
```    
